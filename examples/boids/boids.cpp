...
static constexpr size_t NUM_BOIDS = 20000;
static constexpr real_t SCREEN_WIDTH_PIXELS = 1200.;
static constexpr int FINE_GRAIN_CELL_LIMIT = 1;
static constexpr int COARSE_GRAIN_CELL_LIMIT = 2;
...
struct V2 {
    ...
    inline real_t magnitude(void) const {
        return std::sqrt(x*x + y*y);
    }
};

inline V2 operator*(const V2& v, real_t sf) {
    return {v.x * sf, v.y * sf};
}

inline V2 operator*(real_t sf, const V2& v) {
    return {v.x * sf, v.y * sf};
}
...
inline bool operator==(const V2& lhs, const V2& rhs) {
    return lhs.x == rhs.x && lhs.x == lhs.y;
}
...
inline double wrap_real(real_t x, real_t m) {
...
struct Boid {
    V2 pos;
    V2 vel;
    using Storage = RobinHoodStorage<Boid>;
...
class Grid {
    ...
    class Node {
        ...
        inline real_t population(void) const {
            return (real_t) m_contained.size();
        }

    public:
        Node() {
            m_contained.reserve(256);
        }
        ...
        inline void insert(const Boid& boid) {
            m_contained.push_back(boid);
        }

        inline const PseudoBoid& get_pseudoboid(void) const {
            return m_pseudo_boid;
        }
...
public:
    ...
    void get_pseudoboid_neighbors(const Boid& boid, std::vector<PseudoBoid>& results) const {
        ...
        for (int i = -COARSE_GRAIN_CELL_LIMIT; i <= COARSE_GRAIN_CELL_LIMIT; i = advance_coarse_cell_index(i)) {
            for (int j = -COARSE_GRAIN_CELL_LIMIT; j <= COARSE_GRAIN_CELL_LIMIT; j = advance_coarse_cell_index(j)) {
                const int node_itr = focus_node + N*j + i;
                if (node_itr >= 0 && node_itr < NUM_CELLS) {
...
struct RuleAvgVel {
    V2 delta;
    using Storage = RobinHoodStorage<RuleAvgVel>;
};
...
struct AvgVelRuleSystem {
    ...
    using SystemData = std::tuple< ReadComponent<Boid>
                                 , WriteComponent<RuleAvgVel>
                                 , ReadResource<Grid>
                                 >;

    static constexpr real_t STRENGTH = 3;
    ...
    static void run(FollowedEntities followed, SystemData data) {
       ...
       followed.for_each([&] (const EntityID id) -> void {
           ...
           //FIX: use sum weights intead of popluation() method
...
struct RuleConfine {
    V2 delta;
    using Storage = RobinHoodStorage<RuleConfine>;
};
...
struct RuleDensity {
    V2 delta;
    using Storage = RobinHoodStorage<RuleDensity>;
};
...
struct DensityRuleSystem {
    ...
    static void run(FollowedEntities followed, SystemData data) {
       ...
       followed.for_each([&] (const EntityID id) -> void {
           ...
           // std::cout << "Density Rule Calculation for " << id << ": " << DensityRuleSystem::STRENGTH * c << std::endl;
           density_rule[id].delta = DensityRuleSystem::STRENGTH * c;
           // std::cout << "Density Rule Actual for " << id << ": " << density_rule[id].delta << std::endl;
       });
    }
};
...
struct RuleCOM {
    V2 delta;
    using Storage = RobinHoodStorage<RuleCOM>;
};
...
struct CenterOfMassRuleSystem {
    ...
    using SystemData = std::tuple< ReadComponent<Boid>
                                 , WriteComponent<RuleCOM>
                                 , ReadResource<Grid>
                                 >;

    static constexpr real_t STRENGTH = 2.5;
    ...
    static void run(FollowedEntities followed, SystemData data) {
       ...
       followed.for_each([&] (const EntityID id) -> void {
           ...
           // std::cout << "COM Rule Calculation for " << CenterOfMassRuleSystem::STRENGTH * c / (grid->population() - 1);
           if (sum_weights > 0.) {
               c /= sum_weights;
               com_rule[id].delta = CenterOfMassRuleSystem::STRENGTH * (c-b.pos);
           }
           // std::cout << "COM Rule Actual for " << id << ": " << com_rule[id].delta << std::endl;
       });
    }
};
...
struct PositionUpdateSystem {
    ...
    using SystemData = std::tuple< WriteComponent<Boid>
                                 , ReadComponent<RuleCOM>
                                 , ReadComponent<RuleDensity>
                                 , ReadComponent<RuleAvgVel>
                                 , ReadComponent<RuleConfine>
    ...
    static void run(FollowedEntities followed, SystemData data) {
       ...
       followed.for_each_par([&] (const EntityID id) -> void {
           ...
           // std::cout << "new position after advance: " << id << " with pos: " << b.pos.x << " " << b.pos.y << std::endl;
       });
...
using BoidSystems = TypeList<CenterOfMassRuleSystem,
                             DensityRuleSystem,
                             AvgVelRuleSystem,
...
static void tick(void) {
    ...
    g_world->run_systems_parallel<DensityRuleSystem,
                                  AvgVelRuleSystem,
                                  CenterOfMassRuleSystem,
                                  ConfineRuleSystem>();
...
int main(int argc, char** argv) {
    return boids::boids_main(argc, argv);
}
