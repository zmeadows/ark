...
#define ARK_LOG_EVERYTHING(MSG) do { } while (false)
#define ARK_LOG_VERBOSE(MSG) do { } while (false)
#define ARK_LOG_DEBUG(MSG) do { } while (false)
#define ARK_LOG_INFO(MSG) do { } while (false)
#define ARK_LOG_WARNING(MSG) do { } while (false)

#define __SHORT_FILENAME__ (strrchr(__FILE__, '/') ? strrchr(__FILE__, '/') + 1 : __FILE__)

#ifndef ARK_LOG_DISABLE_COLOR
#define __DO_ARK_LOG(COLOR, label, MSG) std::cout << COLOR("ark log (" label ")") << " " \
                                                << __SHORT_FILENAME__ << "::" \
                                                << __FUNCTION__ << ":" \
                                                << __LINE__ << " -> " \
                                                << MSG << std::endl;
#else
#define __DO_ARK_LOG(COLOR, label, MSG) std::cout << "ark log (" label ")" << " " \
                                                << __SHORT_FILENAME__ << "::" \
                                                << __FUNCTION__ << ":" \
                                                << __LINE__ << " -> " \
                                                << MSG << std::endl;
