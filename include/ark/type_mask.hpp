...
public:
    ...
    {
        m_bitset.set(type_list::index<T, Types>());
    }

    template <typename T>
    inline constexpr void unset(void)
    {
        m_bitset.unset(type_list::index<T, Types>());
    }

    template <typename T>
    inline constexpr bool check(void)
    {
        return m_bitset.test(type_list::index<T, Types>());
    }
    ...
    inline const Bitset2::bitset2<Types::size>& get_bitset(void) const { return m_bitset; }
};
